...
class SlurmDump(Command):
    ...
    def get_parser(self, prog_name):
        ...
        p.add_argument(
            "--date", help="import jobs from this date", default="2023-10-18"
        )
        p.add_argument(
            "--starttime", help="start time of job imports", default="00:00:00"
        )
...
class SlurmImport(Command, GraphQlClient):
    ...
    def get_parser(self, prog_name):
        ...
        p.add_argument(
            "--output", help="output format", choices=["json", "upload"], default="json"
        )
        p.add_argument(
            "--password-file",
    ...
    def take_action(self, parsed_args):
        ...
        self.back_channel = self.connect_graph_ql(
            ...
            password_file=parsed_args.password_file,
            timeout=300,
        )
        self.get_metadata()
    ...
    def get_metadata(self) -> bool:
        ...
        repos = {
            x["facility"].lower() + ":" + x["name"].lower(): x for x in resp["repos"]
        }
    ...
    def get_alloc_id(
        self, facility: str, repo: str, cluster: str, time: pdl.DateTime
    ) -> str:
        ...
        if key in self._allocid:
            ...
            for t, _id in time_ranges.items():
                self.LOG.debug(
                    f"    matching {t[0].isoformat()} <= {time.in_timezone(_now.timezone).isoformat()} < {t[1].isoformat()}?"
                )
                if time >= t[0] and time < t[1]:
                    self.LOG.debug(f"      found match, returning alloc id {_id}")
                    return _id
        raise Exception(
            f"could not determine alloc_id for {facility}:{repo} at {cluster} at timestamp {time}"
        )
    ...
    def upload_jobs(
        ...
        duration = e - s
        self.LOG.info(
            f"imported jobs Inserted={result['insertedCount']}, Upserted={result['upsertedCount']}, Deleted={result['deletedCount']}, Modified={result['modifiedCount']} in {duration:,.02f}s"
        )
        return True
    ...
    def convert(
        self, index, parts, default_facility="shared", default_repo="default"
    ) -> dict:
            ...
            if m:
                ...
                if g:
                    ...
                    if p >= 0:
                        ...
                        raise Exception(
                            "Can't handle multiplier=%s for value=%s" % (g, s)
                        )
                return int(float(m.group(1)) * mul)
            else:
                raise Exception("Can't parse %s" % s)

        def calc_resource_hours(
            startTs: pdl.DateTime,
            endTs: pdl.DateTime,
            tres: str,
            cluster: dict,
            ...
            for resource in ("cpu", "gpu", "mem"):
                if resource in used:
                    ...
                    self.LOG.debug(
                        f"    {resource}: used {used[resource]} / {cluster[resource]}\t -> {ratios[resource]:.5}"
                    )

            compute_time = (
                elapsed_secs * ncpus / 3600.0
            )  # this woudl be the normal calc

            resource_time = (
                elapsed_secs * alloc_nodes * max_ratio * cluster["cpu"] / 3600.0
            )
            if self.verbose:
                print(
                    f"  calc time: {elapsed_secs}s\t compute_hours: {resource_time:.5}\t core_hours: {compute_time:.5}"
                )
        ...
        if d["Partition"] in self._clusters:
            ...
            resource_hours, elapsed_secs = calc_resource_hours(
                ...
                ncpus=ncpus,
                cluster=self._clusters[d["Partition"]],
            )
        else:
            resource_hours = 0.0
            self.LOG.warn(
                f"partition {d['Partition']} not defined in coact, ignoring job"
            )
        ...
        if not qos in ("scavenger", "preemptable", "normal"):
            self.LOG.warning(
                f"could not determine appropriate qos '{d['QOS']}': line {d}"
            )
...
class SlurmRecalculate(Command, GraphQlClient):
    ...
    def get_parser(self, prog_name):
        ...
        p.add_argument(
            "--date", help="recalculate jobs from this date", default="2023-10-18"
        )
        p.add_argument("--verbose", help="verbose output", required=False)
        p.add_argument(
            "--username",
    ...
    def take_action(self, parsed_args):
        ...
        result = self.back_channel.execute(
            gql(
                'mutation update { jobsAggregateForDate(thedate: "'
                + parsed_args.date
                + 'T08:00:00.0000Z" ){ status } }'
            )
        )
...
class Overage(Command, GraphQlClient):
    ...
    def get_parser(self, prog_name):
        ...
        p.add_argument(
            "--windows",
            help="time windows to collate overage calculations",
            type=int,
            nargs="+",
            required=False,
            default=[15, 60, 10080, 43800],
        )
        p.add_argument(
            "--threshold",
            help="percentage at which to be considered over allocatoin",
            type=float,
            required=False,
            default=100.0,
        )
        p.add_argument(
            "--dry-run",
            help="do not actually enforce job holding",
            required=False,
            action="store_true",
            default=False,
        )
    ...
    def get_data(
        ...
        windows: List[str],
        timeout: int = 300,
        per_window_template: Template = Template(
        ...
        for w in windows:
            all_windows.append(
                per_window_template.safe_substitute(minutes=w, key=f"{w:0>6}")
            )
        self.LOG.debug(f"{all_windows}")

        self.back_channel = self.connect_graph_ql(
            username=username, password_file=password_file, timeout=timeout
        )
        query = "query usage {"
        query += "\n".join(all_windows) + ",\n"
    ...
    def format_data(self, result: dict) -> dict:
        ...
        for l in subprocess.check_output(cmd.split()).split(
            b"\n"
    ...
    @time_function
    def overaged(self, data: dict, threshold: float = 100.0) -> List[dict]:
        ...
        for fac, d in data.items():
            ...
            for clust, m in d.items():
                ...
                if len(percentages) > 0:
                    self.LOG.info(
                        f"{fac:16} {clust:12} held={m['held'] if not m['held'] == None else '-':1} over={over:1} change={change:1}   {values}"
                    )
    ...
    def toggle_job_blocking(
        ...
        **xargs,
        ...
        xargs["nodes"] = (
            0 if xargs["over"] else -1
        )  # clear with -1, set to zero to block new jobs
    ...
    def take_action(self, parsed_args):
        ...
        for facility_over in self.overaged(data, threshold=parsed_args.threshold):
            self.toggle_job_blocking(
                execute=False if parsed_args.dry_run == True else True, **facility_over
            )
